...
class MonthHistoryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Month History')),
      body: settlementsAsync.when(
        data: (settlements) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                child: ListTile(
                  title: Text(_monthYear(settlement.year, settlement.month)),
                  subtitle: Text(
                    '${settlement.totalMeals} meals · ${settlement.totalExpense.format()} · '
                    'Rate ${settlement.mealRate.format()}',
                  ),
                  trailing: Chip(
                    label: Text(settlement.isClosed ? 'Closed' : 'Open'),
                    avatar: Icon(settlement.isClosed ? Icons.lock_outline : Icons.lock_open, size: 16),
                  ),
                  onTap: () {
                    ref
        ...
        error: (_, _) => const Center(child: Text("Couldn't load month history.")),
      ),
    );
  }
}
