...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _renameMess() async {
    ...
    if (newName == null || newName.isEmpty || newName == widget.mess.name) return;
    await ref.read(messRepositoryProvider).updateMess(widget.mess.copyWith(name: newName));
  }
  ...
  Future<void> _exportData() async {
    ...
    try {
      ...
      final file = File('${directory.path}/mess_fellows_backup_$timestamp.json');
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Backup imported successfully.')));
      }
        ...
        child: ListView(
          children: [
            ...
            ListTile(
              ...
              subtitle: Text('${widget.mess.currencySymbol} ${widget.mess.currencyCode}'),
            ),
            ListTile(
              ...
              onTap: () => Navigator.of(
                context,
              ).push(MaterialPageRoute(builder: (_) => MembersScreen(messId: widget.mess.id))),
            ),
            const Divider(),
            ...
            ListTile(
              ...
              subtitle: const Text('Save a backup file you can share or store'),
              onTap: _exportData,
            ),
            ListTile(
              leading: const Icon(Icons.download_outlined),
              title: const Text('Import data'),
              subtitle: const Text('Replace local data from a backup file'),
              onTap: _importData,
            ),
            ListTile(
            ...
            const ListTile(
              ...
              subtitle: Text('Version 1.0.0 · Local-first, no account required'),
            ),
          ],
        ),
      ),
