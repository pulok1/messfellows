...
final monthCalculationProvider = Provider.family<MonthCalculationResult, MonthParams>((
  ref,
  params,
) {
  final allMembers = ref.watch(allMembersProvider(params.messId)).value ?? const [];
  final meals =
      ref
          .watch(
            mealsForMonthProvider((messId: params.messId, year: params.year, month: params.month)),
          )
          .value ??
      const [];
  final expenses =
  ...
  final relevantMembers = allMembers
      .where((member) => member.isActive || memberIdsWithActivity.contains(member.id))
