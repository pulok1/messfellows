...
final settlementsProvider =
    StreamProvider.family<List<MonthlySettlement>, String>((ref, messId) {
...
final settlementForMonthProvider =
    Provider.family<MonthlySettlement?, MonthParams>((ref, params) {
      final settlements =
          ref.watch(settlementsProvider(params.messId)).value ?? const [];
      for (final settlement in settlements) {
        if (settlement.year == params.year &&
            settlement.month == params.month) {
          return settlement;
        }
      }
      return null;
...
final settlementMembersProvider =
    StreamProvider.family<List<MonthlySettlementMember>, String>((
      ref,
      settlementId,
    ) {
      return ref
          .watch(settlementRepositoryProvider)
          .watchSettlementMembers(settlementId);
    });
